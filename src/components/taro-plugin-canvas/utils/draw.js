import { downloadImageAndInfo } from "./tools";
...
export function _drawSingleText(drawData, drawOptions) {
  ...
  const { type = "", marginLeft = 0 } = addonAfter;
  ...
  let textWidth = _getTextWidth(drawData, drawOptions);
  const textArr = [];
  let drawWidth = toPx(width || 0);
  let drawMaxWidth = toPx(maxWidth || 0);
  let afterWidth = x + textWidth + marginLeft;
  if ((width || maxWidth) && textWidth > (width || maxWidth)) {
    // 文本宽度 大于 渲染宽度
    let fillText = "";
    let line = 1;
    for (let i = 0; i <= text.length - 1; i++) {
      ...
      if (ctx.measureText(fillText).width >= (drawWidth || drawMaxWidth)) {
        if (line === lineNum) {
          if (i !== text.length - 1) {
    ...
    afterWidth = x + textWidth + marginLeft + 13; // 超长...的距离偏移
  } else {
    textArr.push(text);
  }
  ...
  if (type === "text") {
    _drawSingleText({ ...addonAfter, y, x: afterWidth }, drawOptions);
  }
  // if (type === "image") {
  //   const { toRpx } = drawOptions;
  //   downloadImageAndInfo(addonAfter, 1, toRpx, 1).then((result) => {
  //     drawImage(result, drawOptions);
  //   });
  // }
  return textWidth;
}
