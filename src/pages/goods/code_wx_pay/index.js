...
class Index extends Component {
  ...
  payOrder(res) {
    const {httpData: {orderSn,payMonth}} = this.state
    payOrder({orderSn: orderSn,payMonth, payType: 'wx_lite', wechatCode: res}, result => {
    ...
    return (
      <View className='code_wx_box'>
        <View className='code_wx_content'>
          ...
          <View className='code_pay_money'><Text className='font36'>¥ </Text>{payFee}</View>
        ...
        <PayGo content={`微信支付¥${payFee}`} click={() => this.payScan()}></PayGo>
      </View>
    )
  }
}
