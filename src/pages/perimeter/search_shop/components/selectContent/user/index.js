...
const UserView = ({ keyword, current }) => {
        ...
        if (introduction.length > 0)
          return (
            <View className="search_user_details">
              <View className="search_users_init">
                <View className="search_user_name font_hide">{username} </View>
                <Image
                  className="userLevel_Image"
                  src={userLevelImage}
                  lazyLoad
                  mode="scaleToFill"
                ></Image>
          ...
          return (
            <View className="search_user_details public_center">
              <View className="search_users_init" style={{ paddingTop: 0 }}>
                <View className="search_user_name font_hide">{username} </View>
                <Image
                  className="userLevel_Image"
                  src={userLevelImage}
      ...
      return (
        <View className="search_user_box">
          <View className="search_user_content">
            <View
              className="search_user_profile dakale_profile"
              style={profile ? backgroundObj(profile) : {}}
            ></View>
            {templateContent()}
            {userFollowStatus !== "1" ? (
              <View
                className="search_user_btn search_user_btnStyle1"
                onClick={() =>
                  saveFollow(
                    {
                      followType: "user",
              ...
              <View
                ...
                className="search_user_btn search_user_btnStyle2"
              >
                取消关注
              </View>
            )}
  ...
  return (
    <View style={current == 3 ? { display: "block" } : { display: "none" }}>
      <View className="flex_auto">
        {list.length > 0 ? (
          <View className="search_shopPubu">{createView(list)}</View>
        ) : (
