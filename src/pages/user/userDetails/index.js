...
import {navigateTo,filterStrList,computedHeight,backgroundObj,imgList,filterTime,toast} from '@/common/utils'
...
class Index extends Component {
      ...
      return (
        <View className= 'userDetails_falls_details'>
          <View className='userDetails_falls_makebg'
                style={{...backgroundObj(frontImage),height:Taro.pxTransform(computedHeight(frontImageWidth,frontImageHeight,335))}}>
            {contentType == 'video'?
              ...
              <View className='userDetails_share_videoTag'>
                ...
                <View className='userDetails_share_imgfont'>{imgList(imageContent,'0.0','key').length}</View>
              </View>
            }
            <View className='userDetails_share_accress'>
              <View className='userDetails_share_limitIcon'></View>
              <View className='userDetails_share_limit'>200m {merchantName||''}  </View>
            </View>
          </View>
          <View className='userDetails_share_about'>
            ...
            <View className='userDetails_share_aboutUser'>
              ...
              <View className='userDetails_share_status'>
                <View className={classNames('status_box' && userLikeStatus=='1'?'userDetails_share_icon2':'userDetails_share_icon1')}>
                </View>
                {likeAmount}
              </View>
            </View>
  ...
  //获取个人足迹
  onReachBottom() {
    const {page, limit, countStatus} = this.state
    if (countStatus) {
      this.setState({
        page: page + 1
      }, res => {
  ...
  currentTabs(index) {
    ...
    this.setState({current:index},res => {this.currentTabSwitch(index,[])})
  }
  currentTabSwitch(index,list){
   switch (index) {
     case 0: return  this.setState({page:'1',publicList:list,countStatus:true},res =>{this.getShare()})
     case 1: return  this.setState({page:'1',publicList:list,countStatus: true},res =>{this.getUserCollection()})
     case 2: return  this.setState({page:'1',publicList:list,countStatus: true},res =>{this.getFollowByUserId()})
     case 3: return  this.setState({page:'1',publicList:list,countStatus: true},res =>{this.getUserMark()})
   }
  }
  errorToast(e) {
    this.setState({
      ...
      <View className="userDetails_box">
        <View className="userDetails_top">
              ...
              <View className='userDetails_sex_tags'>
                <View className='userDetails_sex_box'>
                  <View className='userDetails_sex_style userDetails_sex_tagStyle'>
                    <View className={classNames(gender==='M'?'userDetails_sex_girl':'userDetails_sex_boy')}></View>
                  </View>
                  ...
                  {!status&&tag && <View className='userDetails_sex_style userDetails_showStyle' onClick={() =>{this.setState({status: true})}}>
                    <View className='userDetails_show'></View>
                  </View>}
                  {status&&tag.length>0&&<View className='userDetails_sex_style userDetails_hideStyle' onClick={() =>{this.setState({status: false})}}>
          ...
          <View className='userDetails_code'>
            <View className='userDetails_follow' onClick={() =>navigateTo(`/pages/kol/fans/index?userId=${routerId||Taro.getStorageSync('userInfo').userIdString}`)}>
              <View className='userDetails_num'>{userFansNum||0}</View>
              <View className='userDetails_title'>粉丝</View>
            </View>
            <View className='userDetails_fans' onClick={() =>navigateTo(`/pages/kol/follow/index?userId=${routerId||Taro.getStorageSync('userInfo').userIdString}`)}>
            ...
            {(type == 'share' && routerId!==adminId)||(type !='share') && <View className='userDetails_editBtn'>编辑资料</View>}
          </View>
        </View>
        <View className="userDetails_content">
            ...
            {ViewTabs.map((item,index) =>{
              return (
                <View className='userDetails_tabs_child' key={index} onClick={() =>this.currentTabs(index)}>
                  <View className = {classNames('userDetails_tabs_font', parseInt(current)==index&& 'userDetails_tabs_check')}>{item}</View>
                  <View  className = {classNames('userDetails_tabs_line', parseInt(current)!=index && 'userDetails_tabs_lineStyle')}></View>
                </View>
              )
            })}
          </View>
          <View className='userDetails_falls'>
            ...
            {current=='3' && this.createdShareMerchant(publicList)}
           </View>
        </View>
      </View>
    )
