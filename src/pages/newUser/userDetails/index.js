...
class Index extends Component {
  ...
  getFollow() {
    ...
    const {
      userInfo,
      userInfo: { userIdString, userType },
    } = this.state;
    saveFollow(
      {
        followType: userType,
  ...
  createdShopMerchant = (item) => {
    ...
    return (
      <View
        ...
        onClick={() => {
          navigateTo(
            `/pages/perimeter/merchantDetails/index?merchantId=${
              merchantIdString || userMerchantIdString
            }`
          );
        }}
      >
        <View
          className="userDetails_falls_bg"
          style={{ ...backgroundObj(coverImg || coverImage) }}
        >
        ...
        <View className="userDetails_falls_desc">
          ...
          {markFlag == "1" && (
            <View className="userDetails_falls_getBean">
              到店打卡可捡{markBean}
            </View>
          )}
          {couponTitlesJson && (
            <View className="userDetails_falls_coupon">
              {couponTitlesJson.map((item, index) => {
                return (
                  <View className="userDetails_coupon_mj userDetails_coupon_box">
                    {item.couponTitle}
                  </View>
                );
              })}
          ...
          <View className="userDetails_falls_accress">
            <View className="userDetails_falls_city">
              ...
              <View className="userDetails_falls_cityName">{address}</View>
            </View>
    ...
    switch (index) {
      case 0:
        return this.setState(
          { page: 1, publicList: list, countStatus: true },
          (res) => {
            this.getShare();
          }
        );
  ...
  linkTo(item) {
    const { current, routerId, adminId } = this.state;
    if (current == 0) {
      const { momentIndex } = item;
    ...
    return (
      <View className="userDetails_box">
        <View
          style={
            backgroundImg
              ? { ...backgroundObj(backgroundImg) }
              : {
                  background:
                    'url("https://dakale-wechat-new.oss-cn-hangzhou.aliyuncs.com/miniprogram/image/wechatBg.png") no-repeat center/cover',
                }
          }
        ...
        >
          <View className="userDetails_top">
            ...
            <View className="userDetails_user">
              ...
              <View className="userDetails_decBox">
                <View className="userDetails_follow_box">
                  <View className="userDetails_userName">{username}</View>
                  {level > 0 && (
                  ...
                  <View className="userDetails_sex_box">
                    <View className="userDetails_sex_style userDetails_sex_tagStyle">
                      <View
                        className={classNames(
                          gender === "F"
                            ? "userDetails_sex_girl"
                            : "userDetails_sex_boy"
                        )}
                      ></View>
                    </View>
                    {filterStrList(tag).length > 0 ? (
                      filterStrList(tag).map((item, index) => {
                        if (status) {
                          return (
                            <View
                              className="userDetails_sex_style userDetails_tag_style"
                              key={index}
                            >
                              {item}
                            </View>
                    ...
                    {!status && tag && filterStrList(tag).length > 3 && (
                      <View
                        className="userDetails_sex_style userDetails_showStyle"
                        onClick={() => {
                    ...
                    {status && tag.length > 0 && (
                      <View
                        className="userDetails_sex_style userDetails_hideStyle"
                        onClick={() => {
            ...
            <View className="font_noHide userDetails_decs">{introduction}</View>
            <View className="userDetails_code">
              <View
                className="userDetails_follow"
                onClick={() =>
                  navigateTo(
                    `/pages/kol/fans/index?userId=${
                      routerId || Taro.getStorageSync("userInfo").userIdString
                    }`
                  )
                }
              >
                <View className="userDetails_num">
                  {setPeople(userFansNum) || 0}
                </View>
                <View className="userDetails_title">粉丝</View>
              </View>
              <View
                className="userDetails_fans"
                onClick={() =>
                  navigateTo(
                    `/pages/kol/follow/index?userId=${
                      routerId || Taro.getStorageSync("userInfo").userIdString
                    }`
                  )
                }
              ...
              {type == "share" && routerId !== adminId ? (
                userFollowStatus == "1" ? (
                  ...
                  >
                    <View className="userDetails_edit_icon"></View>
                    关注
                  </View>
                )
              ) : (
        ...
        <View className="userDetails_content">
          <View className="userDetails_tabs">
            {ViewTabs.map((item, index) => {
              return (
                <View
                  className="userDetails_tabs_child"
                  key={index}
                  onClick={() => this.currentTabs(index)}
                >
                  <View
                    className={classNames(
                      "userDetails_tabs_font",
                      parseInt(current) == index && "userDetails_tabs_check"
                    )}
                  >
                    {item}
                  </View>
                  <View
                    className={classNames(
                      "userDetails_tabs_line",
                      parseInt(current) != index && "userDetails_tabs_lineStyle"
                    )}
                  ></View>
                </View>
              );
