...
class Index extends Component {
  ...
  goodsPay(res) {
    ...
    payDelayOrder(
      { orderSn: orderSn, payType: "wx_lite", wechatCode: res, token },
      (result) => {
  ...
  componentWillMount() {
    if (
      !getCurrentInstance().router.params.orderSn ||
      !getCurrentInstance().router.params.orderType
    ) {
  ...
  render() {
    const {
      orderResult: { payFee, merchantName, orderSn },
      payStatus,
      payStatus: { value },
      payMonth,
      orderType,
      ...
      0: (
        <>
          <Button
            ...
            className="page_back_btn font32"
          >
            返回APP
          </Button>
          <View
            className="pay_wx_lite_btn public_center"
            onClick={() => this.payByKol()}
          >
            <View className="pay_wx_lite_btnGo font32 bold color6">
              立即支付
            </View>
          </View>
        </>
      ...
      1: (
        <>
          <Button
            ...
            className="page_back_bottomBtn color4 bold font32"
          >
            返回APP
          </Button>
        </>
    ...
    return (
      <View className="pay_wx_lite">
        {value === "1" && <View className="pay_wx_successIcon"></View>}
        <View
          className={classNames(
            "color1 bold",
            value === "0" ? "pay_wx_price" : "pay_wx_successMargin"
          )}
        >
        ...
        <View className="pay_wx_goodsBox">
          <View className="pay_wx_goods">
            <View className="public_auto pay_wx_goodsDetails">
              ...
              <View className="color2 font28 font_hide pay_wx_maxWidth">
                {merchantName}
              </View>
            </View>
            <View className="public_auto pay_wx_goodsDetails">
