...
const favourable = (props) => {
  ...
  const goShopDetails = (merchantId, specialActivityIdString) => {
    navigateTo(
      `/pages/perimeter/favourableDetails/index?merchantId=${merchantId}&specialActivityId=${specialActivityIdString}`
    );
  };
  const goFavourble = () => {
  ...
  if (data && data.length > 0) {
    ...
    const SwiperObj = {
      0: (item, index) => {
        ...
        return (
          <View
            onClick={() => goMerchant(userMerchantIdString)}
            className={classNames(
              "permerter_favourable_lunboImg",
              count === index ? "le-active" : "le-img"
            )}
          >
            <View className="swper_paddings">
              <View className="permerter_favourable_binssge">
                <View
                  className="permerter_favourable_merchantProfile dakale_nullImage"
                  style={{ ...backgroundObj(coverImg) }}
                ></View>
                <View className="permerter_favourable_merchantDetails">
                  <View className="permerter_merchantDetails_name font_hide color1 bold">
                    {merchantName}
                  </View>
                  <View className="permerter_merchantDetails_type  color2 font24">
                    ...
                    <View className="permerter_merchantDetails_font">
                      {categoryName}
                    </View>
                    <View className="permerter_merchantDetails_liner"></View>
                    <View className="permerter_merchantDetails_font">
                      人均￥{perCapitaConsumption || 0}
                    </View>
                  </View>
                  {businessHub && (
        ...
        return (
          ...
          >
            <View className="permerter_favourable_goods">
              <View
                style={{ ...backgroundObj(item.goodsImg) }}
              ...
              >
                ...
                <View className="permerter_favourable_bottom main_fire">
                  <View
                    className="permerter_favourable_userProfile dakale_profile"
                    style={{ ...backgroundObj(item.merchantLogo) }}
                  ></View>
                  <View className="permerter_favourable_name color6 font24 font_hide">
                    {item.merchantName}
                  </View>
                </View>
              </View>
              <View className="permerter_favourable_right">
                <View className="permerter_fav_title font32 color1 bold font_hide">
                  特价橘子
                </View>
                ...
                <View className="permerter_fav_set color1 font20 font_hide">
                  哒卡乐专享价
                </View>
                ...
                <View className="permerter_fav_bean color3 font20">
                  ({"卡豆" + parseInt(realPrice * 100)})
                </View>
              </View>
            </View>
    ...
    return (
      <View className="permerter_favourable">
        ...
        >
          ...
          <View className="permerter_favourable_dec font24">
            到店打卡捡豆，豆豆抵消费
          </View>
          {swiperList && (
            <>
              <View
                className="permerter_favourable_lunbo"
                onClick={(e) => {
                  e.stopPropagation();
                }}
              >
                <Swiper
                  ...
                  className="permerter_favourable_lunboText"
                  previousMargin={Taro.pxTransform(96)}
                  nextMargin={Taro.pxTransform(96)}
                >
                  {swiperList.map((item, index) => {
                    return <SwiperItem>{SwiperObj(item, index)}</SwiperItem>;
                  })}
                </Swiper>
              </View>
                ...
                {swiperList.map((item, index) => {
                  return (
                    <View
                      ...
                      className={classNames(
                        index == count
                          ? "permerter_near_linerTrue"
                          : "permerter_near_false"
                      )}
                    ></View>
                  );
                })}
        ...
        {data.length > 1 ? (
          <Swiper autoplay circular className="permerter_favourable_swiper">
            {data.map((item) => {
              return (
                <SwiperItem className="permerter_favourable_child">
                  <View
                    className="permerter_favourable_child"
                    style={backgroundObj(item.coverImg)}
                  ></View>
                </SwiperItem>
              );
            })}
