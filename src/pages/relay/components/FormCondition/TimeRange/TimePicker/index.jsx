...
const PickerViewBlock = (props) => {
  const { onSelect = () => {}, inValue, type } = props;

  // 选择后的时间数据
  const [dayIndex, setDayIndex] = useState([]);

  // 年
  const [years] = useState(() => addZero(dayjs().year(), 2020));
  ...
  // 小时
  const [hours] = useState(() => addZero(23));

  // 分钟
  const [min] = useState(() => addZero(59));
  ...
  useEffect(() => {
    ...
    // 当前时
    const nowHoure = dateTime.hour();
    // 当前分
    const nowMin = dateTime.minute();
    // 选择时间
    const nowDayValue = [nowYear - 2020, nowMon, nowDay - 1, nowHoure, nowMin];
    // 初始赋值
  ...
  const saveDataTime = (val) => {
    ...
    const selctHoure = hours[val[3]];
    const selctMin = min[val[4]];
    let day = [];
    let inNum = 31;
    // 判断2月
    if (selctMonths == 2) {
    ...
    setDayIndex(val);
    onSelect(
      `${selctYear}-${selctMonths}-${selctDays} ${selctHoure}:${selctMin}`
    );
  };
